...
struct VariableStore<TVM: HLSLCompatibleAbstractVM> {
    general_variables: HashMap<HLSLVectorName, HLSLVariableRef>,
    inputs: HashMap<HLSLVectorName, HLSLVariableRef>,
    outputs: HashMap<HLSLVectorName, HLSLVariableRef>,
...
impl<TVM: HLSLCompatibleAbstractVM> VariableStore<TVM> {
    ...
    fn remap_generic_name(&mut self, old_name: HLSLVectorName) -> HLSLVectorName {
        use HLSLVectorName::*;
        match old_name {
            ...
            GenericRegister(_) => {
                ...
                HLSLVectorName::GenericRegister(format!("{:>03}", id))
            }
...
impl<TVM: HLSLCompatibleAbstractVM> VariableAbstractMachine<TVM> {
    ...
    fn map_dataspec_to_dataref(
        ...
        if (matches!(old_name, HLSLVectorName::GenericRegister(_)) && force_new_general_purpose_var)
