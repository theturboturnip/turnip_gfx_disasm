...
pub enum AMDILTextError<I> {
    Nom(I, ErrorKind),
    ParseIntError(ParseIntError),
    BadVectorComponent(char),
}
impl<I> ParseError<I> for AMDILTextError<I> {
    fn from_error_kind(input: I, kind: ErrorKind) -> Self {
        AMDILTextError::Nom(input, kind)
    }
...
pub fn parse_lines(data: &str) -> IResult<&str, Vec<Instruction>, AMDILTextError<&str>> {
    let (data, lines) = separated_list0(
        line_ending,
        take_while(|c| c != '\r' && c != '\n' && c != '\0'),
    )(data)?;
    let instrs: Result<Vec<Instruction>, nom::Err<AMDILTextError<&str>>> = lines
        .into_iter()
...
fn parse_line(data: &str) -> IResult<&str, Instruction, AMDILTextError<&str>> {
...
fn parse_arg(data: &str) -> IResult<&str, Arg, AMDILTextError<&str>> {
    // args are either a hex literal or a named arg
    alt((parse_hex_literal, parse_named_arg))(data)
}
fn parse_named_arg(data: &str) -> IResult<&str, Arg, AMDILTextError<&str>> {
    // [a-zA-Z]+[0-9]*
...
fn parse_hex_literal(data: &str) -> IResult<&str, Arg, AMDILTextError<&str>> {
    let (data, _) = tag("0x")(data)?;
    let (data, hex_str) = take_while1(|c: char| c.is_ascii_hexdigit())(data)?;
    let hex = u64::from_str_radix(hex_str, 16)
        .map_err(|e| nom::Err::Error(AMDILTextError::ParseIntError(e)))?;
    Ok((data, Arg::HexLiteral(hex)))
}
fn parse_named_arg_mod(data: &str) -> IResult<&str, ArgMod, AMDILTextError<&str>> {
    alt((parse_index_mod, parse_swizzle_mod))(data)
}
fn parse_index_mod(data: &str) -> IResult<&str, ArgMod, AMDILTextError<&str>> {
    let (data, index_str) =
...
fn parse_swizzle_mod(data: &str) -> IResult<&str, ArgMod, AMDILTextError<&str>> {
    // `.[xyzw_]{1,3}`
    let (data, _) = tag(".")(data)?;
    let swizzle_parser = |data| {
        ...
        let comp = match c {
            ...
            '_' => None,
            _ => return Err(nom::Err::Error(AMDILTextError::BadVectorComponent(c))),
        };
        Ok((data, comp))
    };
